	...
	public boolean initBankAccount(String userName) {
		...
		try {
			...
			while (rs.next()) {
				...
				bankAccount = new BankAccount(account_number, balance);
				return initCustomer();
...
//			private final String sql1 = "select customer_name,email_id,phone_number from customer where user_id = ?";
			...
			while (rs.next()) {
				...
				customer = new Customer(bankAccount, username, name, email_id, phone_number);
				return true;
			}
	...
	private void welcomepage() {
		...
		out.println("<html> <form action=\"LogOut\" method=\"get\">"
				+ " <button style=\"position: absolute; right: 25;\"> Logout </button> "
				+ "</form></html>");
		...
		out.println("<html><body>" + "Phone Number :" + customer.getPhone_number() + "<br></body></html>");
